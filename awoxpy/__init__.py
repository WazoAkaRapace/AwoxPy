...
class awoxAroma:
    ...
    def get_state(self):
        ...
        self.red = struct.unpack("h", readed[0] + "\x00")[0]
        self.green = struct.unpack("h", readed[1] + "\x00")[0]
        self.blue = struct.unpack("h", readed[2] + "\x00")[0]

        readed = self.read(self.rgbBrightnessHandle)
        self.colourBrightness = struct.unpack("h", readed[0] + "\x00")[0]

        readed = self.read(self.whitehandle)
        self.white = struct.unpack("h", readed + "\x00")[0]

        readed = self.read(self.whiteBrightnessHandle)
        self.whiteBrightness = struct.unpack("h", readed[0] + "\x00")[0]
